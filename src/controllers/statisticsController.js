...
let revenue = async (req, res) => {
  try {
    ...
    // compute yearlyTotal and monthlyTotal (based on query params)
    const yearStart = new Date(year, 0, 1);
    const yearEnd = new Date(year + 1, 0, 1);
    const yearOrders = await db.Order.findAll({ where: { createdAt: { [Op.gte]: yearStart, [Op.lt]: yearEnd } }, raw: true });
    const yearlyTotal = yearOrders.reduce((s, o) => s + (Number(o.total) || 0), 0);
    let monthlyTotal = 0;
    if (month >= 1 && month <= 12) {
      const monthStart = new Date(year, month - 1, 1);
      const monthEnd = new Date(year, month, 1);
      const monthOrders = await db.Order.findAll({ where: { createdAt: { [Op.gte]: monthStart, [Op.lt]: monthEnd } }, raw: true });
      monthlyTotal = monthOrders.reduce((s, o) => s + (Number(o.total) || 0), 0);
    }
    ...
    if (view === 'month') {
      ...
      return res.status(200).json({ success: true, labels, totals, monthlyTotal, yearlyTotal, datasets: { total: totals }, dataStatistic: totals, datasttic: totals, message: msg });
    }
